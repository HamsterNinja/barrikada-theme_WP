<template>
    <div class="product-card">
        ...
        <div class="card-cat"><h3>{{ product.categories }}</h3></div>
        <a :href="product.permalink" class="card-name"><h4>{{ product.name }}</h4></a>
        <div class="card-bottom">
            <template v-if="product.sale_price">  
                <div class="card-prices">
                    <p>Цена: <span>{{ product.regular_price }} ₽</span></p>
                    <div class="current-price" v-if="product.regular_price">{{ product.sale_price }} ₽</div>
                </div>
            </template>
            <template v-else>   
